...
llen=x(length(x))-x(1);
j=1;
solveLen=4*(length(xk)-1);
solve=zeros(solveLen,solveLen);
...
6*x2^1, 2, 0, 0, -6*x2^2, -2, 0, 0];
plus=4*(i-1);
solve(1+plus:4+plus,1+plus:8+plus)=macrz;
hektor(1+plus:4+plus)=[yk(i),yk(i+1),0,0];
end
solve(solveLen-3:solveLen-2,solveLen-3:solveLen)=[xk(length(xk)-1)^3,xk(length(xk)-1)^2, xk(length(xk)-1)^1, 1;xk(length(xk))^3,xk(length(xk))^2, xk(length(xk))^1, 1];
...
figure; plot(x,y,'r',xk,yk,'ko',x,yll,'b.'); xlabel('x');
grid;
% Cubic spline
yi = interp1(xk,yk,x,'cubic');
figure; plot(x,y,'r',xk,yk,'ko',x,yi,'b.'); xlabel('x');
title('y(x)'); grid;