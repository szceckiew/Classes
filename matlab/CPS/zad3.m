...
title("widmo gestosci mocy oryginalnego sygnalu")
%%%%%%%%%
figure(2);
psd(spectrum.welch('Hamming',1024), wideband_signal_shifted(1:end),'Fs',fs);
title("widmo gestosci mocy pojedynczej stacji")
A_stop = 40;		% t³umienie w pierwszym paœmie zaoprowym
F_stop = 80e3+2000;		% granica pasma zaporowego
F_pass = 80e3;	% granica pasma przepustowego
A_pass = 3;		% zafalowania w paœmie przepustowym nie mog¹ byæ wiêksze ni¿ 3 dB

LowPassSpecObj1 = fdesign.lowpass(F_pass, F_stop, A_pass, A_stop,3.2e6);
LowPassFilt1 = design(LowPassSpecObj1, 'ellip');


[bkek,akek] = tf(LowPassFilt1);


%%%%%%%%%

wideband_signal_filtered = filter( bkek, akek, wideband_signal_shifted );
%wideband_signal_filtered = filter( LowPassFilt1, wideband_signal_shifted );
figure(3);
psd(spectrum.welch('Hamming',1024), wideband_signal_filtered(1:end),'Fs',fs);
title("widmo gestosci przefiltrowanego sygna³u")
